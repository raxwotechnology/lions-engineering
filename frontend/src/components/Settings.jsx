...
const Settings = ({ onSettingsUpdate }) => {
    ...
    try {
      ...
      setSettings({
        ...
        smsBookingTemplate: resolveBookingTemplate(
          ...
          data.companyName || 'LIONS ENGINEERING'
        ),
  ...
  const smsPreview = useMemo(
    () => previewSmsTemplate(settings.smsBookingTemplate, settings.companyName || 'LIONS ENGINEERING'),
  ...
  const resetBookingTemplate = () => {
    const tpl = DEFAULT_SMS_BOOKING_TEMPLATE.replace(
      /\{companyName\}/g,
      settings.companyName || 'LIONS ENGINEERING'
    );
